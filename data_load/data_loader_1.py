...
class subDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.isTrain:
            ...
            target_before = moving_sample[:self.interval]
            target = moving_sample[self.interval:self.interval + self.target_num]
            target_after = moving_sample[self.interval + self.target_num:]
            images = np.concatenate([target_before, target_after, target], axis=0)

        else:
            moving_sample = self.data[:self.seq_len, idx, ...]
            target_before = moving_sample[:self.interval]
            target = moving_sample[self.interval:self.interval + self.target_num]
            target_after = moving_sample[self.interval + self.target_num:]
            images = np.concatenate([target_before, target_after, target], axis=0)
